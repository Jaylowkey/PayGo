...
export default async function handler(req, res) {
  ...
  try {
    // 💡 Agora esta API aceita a ordem direta para enviar também para o LARK
    ...
    if (process.env.RESEND_API_KEY) {
      const { data, error } = await resend.emails.send({
        from: FROM_EMAIL,
        to: [to],
        subject: subject || getFallbackSubject(template, variables), // Se não mandarem assunto, o gerador adivinha
        html: html,
        text: text,
        headers: {
          'X-PayGo-Template': template,
          'X-PayGo-Type': type || 'transactional',
          'X-PayGo-Version': '2.2' // Versão Unificada
        }
      });
    ...
    // ========================================================================
    // 2. ENVIAR NOTIFICAÇÃO PARA O LARK (Opcional, acionado pelas variáveis)
    // ========================================================================
...
// ==========================================
// 🎨 GERADOR DE HTML (O Melhor dos dois mundos)
// ==========================================
function generateEmailHTML(template, vars) {
  ...
  const baseUrl = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : (process.env.NEXT_PUBLIC_SITE_URL || 'https://paygo.co.mz');
  const waLink = getWhatsAppLink(vars.order_id, vars.customer_name, vars.total_amount, vars.payment_method);
  ...
  // CSS Minimalista Padrão (Usado para os e-mails gerais)
  const baseStyles = `<style>@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); body { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #f8fafc; margin: 0; padding: 40px 20px; color: #1e293b; -webkit-font-smoothing: antialiased; } .wrapper { max-width: 520px; margin: 0 auto; background: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 40px -10px rgba(0,0,0,0.08); border: 1px solid #f1f5f9; } .header-gradient { background: linear-gradient(135deg, #2563eb, #06b6d4); padding: 4px; } .logo-container { padding: 32px 32px 16px 32px; text-align: center; } .logo-container h1 { margin: 0; color: #0f172a; font-size: 28px; font-weight: 800; letter-spacing: -1px; } .logo-container h1 span { color: #3b82f6; } .content { padding: 0 32px 32px 32px; } h2 { margin: 0 0 16px 0; font-size: 22px; font-weight: 700; color: #0f172a; line-height: 1.3; text-align: center; } p { margin: 0 0 20px 0; font-size: 15px; line-height: 1.6; color: #475569; } .btn { display: block; background: #0f172a; color: #ffffff !important; padding: 16px 24px; border-radius: 12px; text-decoration: none; font-weight: 600; font-size: 15px; text-align: center; margin: 32px 0; box-shadow: 0 10px 15px -3px rgba(15, 23, 42, 0.1); } .btn-success { background: #16a34a; } .data-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 24px; margin: 24px 0; } .data-card h3 { margin: 0 0 16px 0; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; color: #64748b; } table { width: 100%; border-collapse: collapse; } td { padding: 10px 0; font-size: 14px; border-bottom: 1px dashed #e2e8f0; } tr:last-child td { border-bottom: none; padding-bottom: 0; } .label { color: #64748b; width: 40%; font-weight: 500; } .value { color: #0f172a; font-weight: 600; text-align: right; } .total-row td { padding-top: 16px; border-top: 2px solid #cbd5e1; font-size: 18px; font-weight: 800; } .total-value { color: #16a34a; } .footer { padding: 32px; background: #f8fafc; text-align: center; border-top: 1px solid #e2e8f0; } .footer p { margin: 0 0 8px 0; font-size: 12px; color: #94a3b8; } .brand-claim { font-weight: 700; color: #64748b !important; } a { color: #3b82f6; text-decoration: none; } .code-highlight { background: #f0fdf4; border: 2px dashed #22c55e; color: #166534; padding: 20px; border-radius: 12px; text-align: center; font-size: 24px; font-weight: 800; letter-spacing: 2px; margin: 24px 0; }</style>`;
  ...
  switch (template) {
    ...
    case 'order-completed':
      return `${htmlStart}<h2>Pedido Concluído! 🎉</h2><p>Olá, <strong>${escape(vars.customer_name)}</strong>.</p><p>Boas notícias! O seu pedido <strong>#${escape(vars.order_id)}</strong> foi finalizado com sucesso pela nossa equipa.</p><div class="data-card" style="border-color: #bbf7d0; background: #f0fdf4;"><h3 style="color: #166534;">Resumo Final</h3><table><tr><td class="label" style="color: #166534;">Status</td><td class="value" style="color: #14532d;">Despachado / Concluído ✅</td></tr></table></div><p style="text-align: center; margin-top: 32px;">Se a sua compra incluiu produtos físicos, verifique o código de rastreio na sua conta PayGo.</p><a href="${baseUrl}/login.html" class="btn btn-success">Fazer Nova Compra</a>${htmlEnd}`;

    case 'password-reset':
      return `${htmlStart}<h2>Recuperação de Senha 🔐</h2><p>Olá, <strong>${escape(vars.customer_name)}</strong>.</p><p>Recebemos um pedido para redefinir a palavra-passe da sua conta PayGo. Clique no botão abaixo para criar uma nova senha de forma segura.</p><a href="${escape(vars.reset_link)}" class="btn">Redefinir Palavra-passe</a><p style="font-size: 13px; color: #94a3b8; margin-top: 24px; text-align: center;">Se não solicitou esta alteração, ignore este e-mail. A sua conta permanecerá segura.</p>${htmlEnd}`;

    case 'email-verification':
      const verifyLink = `${baseUrl}/seguranca.html?mode=verifyEmail&oobCode=${vars.verificationToken}&email=${encodeURIComponent(vars.email || '')}`;
      return `${htmlStart}<h2>Confirme o seu e-mail 🛡️</h2><p>Olá, <strong>${escape(vars.customer_name)}</strong>.</p><p>Para garantir a segurança máxima da sua conta PayGo e ativar os seus pagamentos, precisamos que valide este endereço de e-mail.</p><a href="${verifyLink}" class="btn">Validar a Minha Conta</a><p style="font-size: 12px; text-align: center; color: #94a3b8; margin-top: 20px;">Este link de segurança expira em 24 horas.</p>${htmlEnd}`;

    case 'welcome':
      return `${htmlStart}<h2>Bem-vindo à PayGo! 🚀</h2><p>Olá, <strong>${escape(vars.customer_name)}</strong>. A sua conta foi ativada com sucesso e já faz parte da nossa plataforma.</p><p>A partir de agora, o comércio global não tem fronteiras. Compre no mundo todo e pague com o seu M-Pesa ou e-Mola.</p><a href="${baseUrl}/login.html" class="btn">Aceder à Plataforma</a>${vars.affiliate_code && vars.affiliate_code !== 'null' ? `<div style="text-align: center; margin-top: 40px; border-top: 1px solid #e2e8f0; padding-top: 32px;"><p style="font-size: 12px; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.1em;">🎁 O seu Código Promocional</p><div class="code-highlight">${escape(vars.affiliate_code)}</div><p style="font-size: 14px; color: #475569;">Indique amigos usando o seu código e ganhe comissões na primeira compra deles!</p></div>` : ''}${htmlEnd}`;

    default:
      return `${htmlStart}<h2>Notificação Oficial PayGo</h2><div style="background: #f8fafc; padding: 24px; border-radius: 12px; color: #1e293b; line-height: 1.6; border: 1px solid #e2e8f0;">${vars.message || 'Nova notificação administrativa.'}</div><a href="${baseUrl}/login.html" class="btn" style="margin-bottom:0;">Aceder à Conta</a>${htmlEnd}`;
  }
}

// ==========================================
// 📝 GERADOR DE TEXTO (Fallback e Assuntos)
// ==========================================
function generateEmailText(template, vars) {
  ...
  switch (template) {
    ...
    default: return `${(vars.message || '').replace(/<[^>]*>?/gm, '')}${footer}`;
  }
}
...
function getFallbackSubject(template, vars) {
  switch (template) {
    ...
    case 'payment-confirmed': return `✅ Pagamento Recebido - Pedido ${vars.order_id || ''}`;
    case 'order-processing': return `🔄 O seu Pedido ${vars.order_id || ''} está em processamento`;
    case 'order-completed': return `🎉 Pedido ${vars.order_id || ''} Concluído - PayGo`;
    case 'password-reset': return '🔐 Recuperação de Senha - PayGo';
    case 'email-verification': return '🛡️ Verifique o seu e-mail da PayGo';
    case 'welcome': return '🚀 Bem-vindo ao Mundo Global PayGo!';
    default: return 'Notificação PayGo Moçambique';
  }
}
...
async function sendLarkNotification(template, vars) {
  try {
    ...
    // O Lark só é acionado para alertas financeiros
    if (template === 'order-confirmation') {
        ...
        // Se for um template normal (ex: welcome), ignoramos o envio para o Lark
    ...
    const larkPayload = {
      ...
      card: {
        ...
        elements: [
          ...
          { tag: "action", actions: [{ tag: "button", text: { tag: "plain_text", content: "Ver no Painel" }, type: "primary", url: `${baseUrl}/admin/pedidos.html` }] }
        ]
      }
    };
