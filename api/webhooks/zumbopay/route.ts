...
function dataOf(event: any) { return event?.data || event?.payment || event?.transaction || event || {}; }
function eventTypeOf(event: any) { return String(event?.type || event?.event || event?.event_type || "").trim().toLowerCase(); }
function referenceOf(event: any) { const d = dataOf(event); return String(d?.reference || event?.reference || "").trim(); }
function sourceIdOf(event: any) { const d = dataOf(event); return d?.source_id || d?.sourceId || event?.source_id || event?.sourceId || null; }
function paymentIdOf(event: any) { const d = dataOf(event); return d?.payment_id || d?.paymentId || d?.id || d?.transaction_id || d?.transactionId || null; }
function amountOf(event: any) { const d = dataOf(event); const n = Number(d?.amount ?? d?.gross_amount ?? d?.value ?? 0); return Number.isFinite(n) ? n : 0; }
function statusOf(event: any) { return String(dataOf(event)?.status || event?.status || "").trim().toLowerCase(); }
function isSuccess(type: string, status: string) { return ["payment.succeeded", "payment.success", "payment.completed", "charge.succeeded", "charge.success", "charge.completed"].includes(type) || ["success", "succeeded", "completed", "paid"].includes(status); }
function isFailed(type: string, status: string) { return ["payment.failed", "payment.failure", "payment.cancelled", "payment.canceled", "charge.failed", "charge.cancelled", "charge.canceled"].includes(type) || ["failed", "failure", "cancelled", "canceled"].includes(status); }
function isRefunded(type: string, status: string) { return ["payment.refunded", "payment.refund", "charge.refunded"].includes(type) || ["refunded", "refund"].includes(status); }
...
async function recordAttempt(db: Firestore, event: any, extra: Record<string, any> = {}) {
  ...
  await db.collection("payment_attempts").doc(id).set({
    provider: "zumbopay", reference, sourceId: sourceIdOf(event), paymentId: paymentIdOf(event),
    eventType: type, status: statusOf(event), amount: amountOf(event), ...extra, rawEvent: event,
...
async function notify(db: Firestore, userId: string, reference: string, amount: number, type: string, title: string, message: string) {
  await db.collection("notifications").doc(`ZUMBOPAY_${type}_${reference}`).set({
    userId, type, title, message, reference, amount, currency: "MZN", read: false,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let event: any;
    try { event = JSON.parse(rawBody); } catch { return NextResponse.json({ success: false, error: "Invalid JSON" }, { status: 400 }); }
    ...
    if (providerAmount > 0 && Math.abs(providerAmount - amount) > 0.01) {
      await found.ref.update({ status: "amount_mismatch", provider: "zumbopay", providerReference: reference, providerPaymentId: paymentId || null, providerAmount, updatedAt: FieldValue.serverTimestamp() });
    ...
    if (isFailed(type, status)) {
      ...
      if (!["completed", "paid", "refunded"].includes(current)) {
        await found.ref.update({ status: "failed", provider: "zumbopay", providerReference: reference, providerPaymentId: paymentId || null, providerStatus: status, updatedAt: FieldValue.serverTimestamp() });
    ...
    if (isRefunded(type, status)) {
      const current = String(topup.status || "pending").toLowerCase();
      await found.ref.update({ status: "refunded", provider: "zumbopay", providerReference: reference, providerPaymentId: paymentId || null, providerStatus: status, refundPending: current === "completed" || current === "paid", updatedAt: FieldValue.serverTimestamp() });
    ...
    if (!isSuccess(type, status)) return NextResponse.json({ success: true, received: true, ignored: true });
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (["completed", "paid"].includes(currentStatus)) { alreadyProcessed = true; return; }

      const user = userSnap.data() || {};
      before = Number(user.walletBalance || 0);
      after = Math.round((before + amount + Number.EPSILON) * 100) / 100;

      tx.update(userRef, { walletBalance: after, updatedAt: FieldValue.serverTimestamp() });
      tx.update(found.ref, {
        status: "completed", provider: "zumbopay", providerReference: reference,
        providerPaymentId: paymentId || null, providerSourceId: sourceId || null,
        providerStatus: status || "succeeded", providerAmount: providerAmount || amount,
        completedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp(),
      });

      if (!walletTxSnap.exists) {
        tx.create(walletTxRef, {
          userId, type: "credit", category: "deposit", amount, currency: "MZN", provider: "zumbopay",
          reference, sourceId: sourceId || null, paymentId: paymentId || null,
          description: `Depósito via ZumboPay - ${reference}`, balanceBefore: before,
          balanceAfter: after, status: "completed", createdAt: FieldValue.serverTimestamp(),
        });
      }

      tx.set(notificationRef, {
        userId, type: "deposit_success", title: "Depósito confirmado",
        message: `O seu depósito de ${amount.toFixed(2)} MT foi confirmado e já está disponível na sua carteira.`,
        amount, currency: "MZN", reference, paymentId: paymentId || null, read: false,
        createdAt: FieldValue.serverTimestamp(),
      }, { merge: true });
    });

    console.log("[ZumboPay] deposit settled", { userId, topupId, reference, amount, before, after, alreadyProcessed });
    return NextResponse.json({ success: true, received: true, status: "completed", reference, paymentId, amount, credited: !alreadyProcessed, alreadyProcessed });
  } catch (error: any) {
