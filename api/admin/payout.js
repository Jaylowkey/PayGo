...
async function createZumbo(body) {
  ...
  const destination = phone(body.destination || body.phone);
  const walletId = String(body.walletId || body.wallet_id || "").trim();
  if (!walletId) throw new Error("Wallet ZumboPay é obrigatória.");
  if (amount < MIN_PAYOUT) throw new Error(`O payout mínimo é ${MIN_PAYOUT.toFixed(2)} MT.`);
  ...
  const payload = { wallet_id: walletId, amount, method: m, destination, ...(body.notes ? { notes: String(body.notes).slice(0, 500) } : {}) };
  const payout = await requestJson(`${ZUMBO_URL}/payouts`, {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "X-Merchant-Id": merchantId, "Idempotency-Key": idempotencyKey, Accept: "application/json", "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  return {
    id: payout.id || null, reference: payout.reference || null, providerReference: payout.provider_reference || null,
    provider: "zumbopay", method: payout.method || m, destination: payout.destination || destination,
    amount: money(payout.amount ?? amount), fee: money(payout.fee_amount), currency: payout.currency || "MZN",
    status: String(payout.status || "pending").toUpperCase(), walletId,
  };
}
...
async function createNetshop(body) {
  ...
  const m = method(body.method);
  const amount = money(body.amount);
  const msisdn = phone(body.destination || body.phone || body.msisdn);
  if (amount < MIN_PAYOUT) throw new Error(`O payout mínimo é ${MIN_PAYOUT.toFixed(2)} MT.`);
  if (!m || !/^\+258\d{9}$/.test(msisdn)) throw new Error("Método ou número de destino inválido.");
  ...
  const payout = await requestJson(`${NETSHOP_URL}/payouts`, {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "X-Wallet-ID": String(walletId), "Idempotency-Key": idempotencyKey, Accept: "application/json", "Content-Type": "application/json" },
    body: JSON.stringify({ amount, currency: "MZN", method: m, msisdn, reference }),
  });
  return {
    id: payout.id || null, reference: payout.reference || reference, providerReference: payout.provider_reference || payout.payout_id || null,
    provider: "netshop", method: payout.method || m, destination: payout.msisdn || msisdn,
    amount: money(payout.amount ?? amount), fee: money(payout.fee || payout.fee_amount), currency: payout.currency || "MZN",
    status: String(payout.status || "PROCESSING").toUpperCase(), walletId,
  };
}
...
async function persist(provider, payout, body) {
  ...
  await db.collection(collection).doc(id).set({
    ...
    createdAt: new Date(), updatedAt: new Date(), source: "paygo-admin-unified-payout",
  }, { merge: true });
}
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      ...
      return res.status(200).json({ success: true, payouts, providers: { netshop: Boolean(process.env.NETSHOP_API_KEY), zumbopay: Boolean(process.env.ZUMBOPAY_API_KEY) } });
    }
    ...
    await db.collection("admin_audit_logs").add({
      adminAction: "PAYOUT_CREATED", provider, payoutId: payout.id || null, reference: payout.reference || null,
      amount: payout.amount, method: payout.method, destination: payout.destination, createdAt: new Date(),
    });
