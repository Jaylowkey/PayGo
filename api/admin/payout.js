...
const MIN_PAYOUT = Math.max(0, Number(process.env.PAYOUT_MIN_MZN || 100));
...
async function requestJson(url, options = {}) {
  ...
  if (!response.ok) {
    ...
    err.status = response.status; err.code = data?.error?.code || data?.code || null; throw err;
  }
...
function validateCommon(body) {
  ...
  if (amount < MIN_PAYOUT) throw new Error(`O payout mínimo configurado é ${MIN_PAYOUT.toFixed(2)} MT.`);
  if (!m || !/^\+258\d{9}$/.test(destination)) throw new Error("Método ou número de destino inválido.");
...
async function createZumbo(body) {
  ...
  const payout = await requestJson(`${ZUMBO_URL}/payouts`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "X-Merchant-Id": merchantId, "Idempotency-Key": idempotencyKey, Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify({ wallet_id: walletId, amount, method: m, destination, ...(body.notes ? { notes: String(body.notes).slice(0, 500) } : {}) }) });
  return { id: payout.id || null, reference: payout.reference || null, providerReference: payout.provider_reference || null, provider: "zumbopay", method: payout.method || m, destination: payout.destination || destination, amount: money(payout.amount ?? amount), fee: money(payout.fee_amount), currency: payout.currency || "MZN", status: String(payout.status || "pending").toUpperCase(), walletId };
}
async function createNetshop(body) {
  ...
  const payout = await requestJson(`${NETSHOP_URL}/payouts`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "X-Wallet-ID": String(walletId), "Idempotency-Key": idempotencyKey, Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify({ amount, currency: "MZN", method: m, msisdn: destination, reference }) });
  return { id: payout.id || null, reference: payout.reference || reference, providerReference: payout.provider_reference || payout.payout_id || null, provider: "netshop", method: payout.method || m, destination: payout.msisdn || destination, amount: money(payout.amount ?? amount), fee: money(payout.fee || payout.fee_amount), currency: payout.currency || "MZN", status: String(payout.status || "PROCESSING").toUpperCase(), walletId };
}
...
async function history(provider) {
  ...
  try { const snap = await db.collection(collection).orderBy("createdAt", "desc").limit(200).get(); return snap.docs.map(d => serialize(d, provider)); }
  catch (error) { console.warn(`[PayGo Admin Payout] history ${provider}`, error?.message || error); return []; }
}
async function persist(provider, payout, body) {
  const { db } = getFirebase();
  const collection = provider === "netshop" ? "netshop_payouts" : "zumbopay_payouts";
  const id = String(payout.reference || payout.id || `${provider}-${Date.now()}`).replace(/\//g, "_");
  await db.collection(collection).doc(id).set({ ...payout, provider, holder: body.holder ? String(body.holder).slice(0, 160) : null, notes: body.notes ? String(body.notes).slice(0, 500) : null, createdAt: new Date(), updatedAt: new Date(), source: "paygo-admin-unified-payout" }, { merge: true });
}
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      ...
      if (!["all", "netshop", "zumbopay"].includes(provider)) return res.status(400).json({ success: false, error: "Provider inválido." });
      const [netshop, zumbopay] = await Promise.all([provider === "all" || provider === "netshop" ? history("netshop") : [], provider === "all" || provider === "zumbopay" ? history("zumbopay") : []]);
      const payouts = [...netshop, ...zumbopay].sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
      return res.status(200).json({ success: true, payouts, minPayout: MIN_PAYOUT, providers: { netshop: Boolean(process.env.NETSHOP_API_KEY), zumbopay: Boolean(process.env.ZUMBOPAY_API_KEY) } });
    }
    ...
    const { db } = getFirebase();
    await db.collection("admin_audit_logs").add({ adminAction: "PAYOUT_CREATED", provider, payoutId: payout.id || null, reference: payout.reference || null, amount: payout.amount, method: payout.method, destination: payout.destination, createdAt: new Date() });
    return res.status(201).json({ success: true, payout });
  } catch (error) {
    console.error("[PayGo Admin Payout]", error);
    return res.status(error?.status >= 400 && error.status < 500 ? error.status : 502).json({ success: false, error: error?.message || "Não foi possível processar o payout.", code: error?.code || null });
  }
}
