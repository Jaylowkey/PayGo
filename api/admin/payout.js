...
// PayGo keeps its own minimum at 10 MT. For ZumboPay we use the instant
// M-Pesa B2C flow, because the manual flow has a provider-side minimum of 500 MT.
...
const method = (v) => {
  const x = String(v || "").toLowerCase().replace(/[^a-z0-9]/g, "");
  return ["mpesa", "emola", "mkesh", "card"].includes(x) ? x : "";
};

const phone = (v) => {
  const d = String(v || "").replace(/\D/g, "");
  if (/^258\d{9}$/.test(d)) return `+${d}`;
  if (/^\d{9}$/.test(d)) return `+258${d}`;
  return "";
};

const netshopWallet = (m) =>
  process.env[`NETSHOP_WALLET_${m.toUpperCase()}`] || process.env.NETSHOP_WALLET_ID || "";
...
function firebase() {
  if (!getApps().length) {
    ...
    if (a.private_key) a.private_key = a.private_key.replace(/\\n/g, "\n");
...
async function json(url, options = {}) {
  ...
  try {
    ...
    d = { message: t || `HTTP ${r.status}` };
  }
  if (!r.ok) {
    const e = new Error(String(d?.error?.message || d?.message || d?.error || `HTTP ${r.status}`));
    e.status = r.status;
    e.code = d?.error?.code || d?.code || null;
    throw e;
  }
  return d?.data ?? d;
}
...
function validate(body, provider) {
  ...
  const min = MIN_PAYOUT[provider] || 10;

  if (amount < min) {
    throw new Error(`O payout ${provider === "netshop" ? "NetShop" : "ZumboPay"} não pode ser inferior a ${min.toFixed(2)} MT.`);
  }
  ...
  // ZumboPay's manual payout has a 500 MT provider minimum. PayGo therefore
  // uses the documented instant M-Pesa B2C payout for this provider.
  if (provider === "zumbopay" && m !== "mpesa") {
    throw new Error("O payout ZumboPay instantâneo usa M-Pesa B2C. Selecione M-Pesa.");
  }
...
async function createZumbo(body) {
  ...
  if (!key || !merchant) throw new Error("Credenciais ZumboPay não configuradas.");

  const { amount, m, destination } = validate(body, "zumbopay");
  const walletId = String(body.walletId || body.wallet_id || "").trim();
  if (!walletId) throw new Error("Selecione uma wallet ZumboPay.");

  const p = await json(`${ZUMBO_URL}/payouts`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${key}`,
      "X-Merchant-Id": merchant,
      "Idempotency-Key": String(body.idempotencyKey || `paygo-${Date.now()}-${crypto.randomBytes(8).toString("hex")}`),
    ...
    body: JSON.stringify({
      ...
      method: m,
      destination,
      // Important: this changes ZumboPay from manual payout to instant M-Pesa B2C.
      auto_dispatch: true,
  ...
  return {
    ...
    walletId,
    payoutType: p.auto_dispatched ? "instant_b2c" : "manual",
  };
}

async function createNetshop(body) {
  const key = process.env.NETSHOP_API_KEY;
  if (!key) throw new Error("NETSHOP_API_KEY não configurada.");

  const { amount, m, destination } = validate(body, "netshop");
  ...
  const p = await json(`${NETSHOP_URL}/payouts`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${key}`,
      "X-Wallet-ID": String(walletId),
      "Idempotency-Key": String(body.idempotencyKey || `paygo-${reference}-${crypto.randomBytes(6).toString("hex")}`),
      Accept: "application/json",
      "Content-Type": "application/json",
    },
  ...
  return {
    ...
    providerReference: p.provider_reference || p.payout_id || null,
    provider: "netshop",
    method: p.method || m,
    destination: p.msisdn || destination,
    amount: money(p.amount ?? amount),
    fee: money(p.fee || p.fee_amount),
    currency: p.currency || "MZN",
    status: String(p.status || "PROCESSING").toUpperCase(),
    walletId,
  };
}
...
function serialize(doc, provider) {
  const d = doc.data() || {};
  const c = d.createdAt?.toDate ? d.createdAt.toDate().toISOString() : d.createdAt || null;
  return {
    ...d,
    id: doc.id,
    ...
    createdAt: c,
    updatedAt: d.updatedAt?.toDate ? d.updatedAt.toDate().toISOString() : d.updatedAt || c,
  };
}
...
async function history(provider) {
  ...
  try {
    const s = await db.collection(collection).orderBy("createdAt", "desc").limit(200).get();
    return s.docs.map((d) => serialize(d, provider));
  } catch {
    return [];
  }
}
  ...
  try {
    if (req.method === "GET") {
      ...
      return res.status(200).json({
        success: true,
        payouts: [...n, ...z].sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || ""))),
        minPayout: MIN_PAYOUT,
        providers: {
          netshop: Boolean(process.env.NETSHOP_API_KEY),
          zumbopay: Boolean(process.env.ZUMBOPAY_API_KEY),
        },
      });
    }
    ...
    if (!["netshop", "zumbopay"].includes(provider)) {
      return res.status(400).json({ success: false, error: "Provider inválido." });
    }
    ...
    await firebase().collection("admin_audit_logs").add({
    ...
    console.error("[PayGo Admin Payout]", e);
    return res
      .status(e?.status >= 400 && e.status < 500 ? e.status : 502)
      .json({ success: false, error: e?.message || "Não foi possível processar o payout.", code: e?.code || null });
  }
}
