...
function assertConfigured() {
  ...
  if (!apiKey) throw Object.assign(new Error("NETSHOP_API_KEY não configurada."), { code: "PROVIDER_NOT_CONFIGURED", provider: "netshop" });
  if (!walletId) throw Object.assign(new Error("NETSHOP_WALLET_ID não configurada."), { code: "PROVIDER_NOT_CONFIGURED", provider: "netshop" });
}
...
export async function health() {
  try {
    assertConfigured();
    return { provider: "netshop", configured: true, healthy: true };
  } catch (error) {
    return { provider: "netshop", configured: false, healthy: false, error: error.message };
  }
}
