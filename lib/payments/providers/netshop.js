...
function normalizeMethod(method) {
  ...
  if (["visa", "mastercard", "cartao", "cartão"].includes(value)) return "card";
  return value;
}
...
function assertConfigured() {
  const { apiKey, walletId } = config();
  if (!apiKey) throw Object.assign(new Error("NETSHOP_API_KEY não configurada."), { code: "PROVIDER_NOT_CONFIGURED", provider: "netshop", retryable: false });
  if (!walletId) throw Object.assign(new Error("NETSHOP_WALLET_ID não configurada."), { code: "PROVIDER_NOT_CONFIGURED", provider: "netshop", retryable: false });
}
...
async function request(path, options = {}) {
  ...
  try { data = text ? JSON.parse(text) : {}; } catch {
    throw Object.assign(new Error(`Resposta inválida do NetShop (HTTP ${response.status}).`), { provider: "netshop", status: response.status, retryable: response.status >= 500 });
  }

  if (!response.ok) {
    throw Object.assign(new Error(data?.error?.message || data?.message || `NetShop HTTP ${response.status}`), {
      provider: "netshop", status: response.status, code: data?.error?.code || data?.code || null,
      retryable: response.status === 429 || response.status >= 500, providerResponse: data,
    });
  }
  return data;
}
...
export async function health() {
  try { assertConfigured(); return { provider: "netshop", configured: true, healthy: true }; }
  catch (error) { return { provider: "netshop", configured: false, healthy: false, error: error.message }; }
}

export async function createCharge({ amount, method, phone, reference, customerName }) {
  const normalizedMethod = normalizeMethod(method);
  const payload = {
    amount: Number(amount), currency: "MZN", method: normalizedMethod,
