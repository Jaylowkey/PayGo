...
function assertConfigured() {
  const c = config();
  if (!c.apiKey) throw Object.assign(new Error("ZUMBOPAY_API_KEY não configurada."), { code: "PROVIDER_NOT_CONFIGURED", provider: "zumbopay", retryable: false });
  if (!c.merchantId) throw Object.assign(new Error("ZUMBOPAY_MERCHANT_ID não configurada."), { code: "PROVIDER_NOT_CONFIGURED", provider: "zumbopay", retryable: false });
}
...
async function request(path, options = {}) {
  ...
  try { data = text ? JSON.parse(text) : {}; } catch {
    throw Object.assign(new Error(`Resposta inválida da ZumboPay (HTTP ${response.status}).`), { provider: "zumbopay", status: response.status, retryable: false });
  }
  if (!response.ok) {
    throw Object.assign(new Error(data?.error?.message || data?.message || `ZumboPay HTTP ${response.status}`), {
      provider: "zumbopay", status: response.status, code: data?.error?.code || data?.code || null,
      retryable: false, providerResponse: data,
    });
  }
  return data;
}
...
export async function health() {
  try { assertConfigured(); return { provider: "zumbopay", configured: true, healthy: true }; }
  catch (error) { return { provider: "zumbopay", configured: false, healthy: false, error: error.message }; }
}
...
export async function createCharge({ amount, method: rawMethod, phone, reference, customerName }) {
  ...
  const response = await request("/charges", {
    ...
    body: JSON.stringify({ wallet_id: walletId, amount: Number(amount), msisdn, customer_name: customerName || "Cliente PayGo", source_id: source }),
  });
  const data = response?.data || {};
  return { success: true, provider: "zumbopay", paymentId: data?.id || data?.payment_id || null, reference: data?.reference || source, status: data?.status || "pending", amount: Number(data?.amount ?? amount), method: m, raw: response };
}
